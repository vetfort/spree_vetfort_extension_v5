  ...
  describe '#fetch' do
    ...
    context 'when LLM returns no matching tags' do
      ...
      it 'falls back to text search' do
        active = double('ActiveRecord::Relation')
        joined_variants = double('ActiveRecord::Relation')
        joined_translations = double('ActiveRecord::Relation')
        where_scope = double('ActiveRecord::Relation')
        distinct_scope = double('ActiveRecord::Relation')
        limited_scope = []

        allow(Spree::Product).to receive(:active).and_return(active)
        allow(active).to receive(:joins).with(:variants_including_master).and_return(joined_variants)
        allow(joined_variants).to receive(:joins).with(/LEFT OUTER JOIN/).and_return(joined_translations)
        allow(joined_translations).to receive(:where).and_return(where_scope)
        allow(where_scope).to receive(:distinct).and_return(distinct_scope)
        allow(distinct_scope).to receive(:limit).with(5).and_return(limited_scope)
