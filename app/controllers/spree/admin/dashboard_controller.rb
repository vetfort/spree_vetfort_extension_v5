  ...
  module Admin
    class DashboardController < BaseController
      ...
      def load_top_products
        ...
        if @vendor.present?
          line_items_grouped = line_items_grouped.joins(:product).where('spree_products.vendor_id': @vendor.id)
        end
      ...
      def load_analytics_data
        ...
        @audience = if same_day?
                      @audience_scope.group_by_hour(:created_at, range: analytics_time_range,
                                                                 time_zone: current_store.preferred_timezone, default_value: 0)
                    else
                      @audience_scope.group_by_day(:created_at, range: analytics_time_range,
                                                                time_zone: current_store.preferred_timezone, default_value: 0)
                    end
        ...
        @visits = if same_day?
                    @visits_scope.group_by_hour(:started_at, range: analytics_time_range,
                                                             time_zone: current_store.preferred_timezone, default_value: 0)
                  else
                    @visits_scope.group_by_day(:started_at, range: analytics_time_range,
                                                            time_zone: current_store.preferred_timezone, default_value: 0)
                  end
