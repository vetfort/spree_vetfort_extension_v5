...
export default class extends CheckboxSelectAll {
  ...
  initialize() {
    // Ensure ignoredVariants is available before any value change callbacks run
  ...
  variantsValueChanged() {
    ...
    if (!this.ignoredVariants) this.ignoredVariants = new Set()
  ...
  createInputsForVariant(keys, variant, index) {
    const optionDefByName = (name) => {
      const all = Object.entries(this.optionsValue || {});
      const found = all.find(([, v]) => v && v.name === name);
    ...
    keys.forEach((key) => {
      const def = optionDefByName(key) || {};
      {
        const el = document.createElement('input');
        el.type = 'hidden';
        el.name = `product[variants_attributes][${index}][options][][id]`;
        el.value = def.id || "";
        inputs.push(el);
      }
      {
        const el = document.createElement('input');
        el.type = 'hidden';
        el.name = `product[variants_attributes][${index}][options][][name]`;
        el.value = key;
        inputs.push(el);
      }
      {
        const el = document.createElement('input');
        el.type = 'hidden';
        el.name = `product[variants_attributes][${index}][options][][position]`;
        el.value = def.position != null ? String(def.position) : "";
        inputs.push(el);
      }
      {
        const el = document.createElement('input');
        el.type = 'hidden';
        el.name = `product[variants_attributes][${index}][options][][option_value_name]`;
        el.value = (variant[key]?.value ?? "");
        inputs.push(el);
      }
      {
        const el = document.createElement('input');
        el.type = 'hidden';
        el.name = `product[variants_attributes][${index}][options][][option_value_presentation]`;
        el.value = (variant[key]?.text ?? "");
        inputs.push(el);
      }
    });

    const resolveKey = (name, map) => {
      if (!map || !name) return null;
      const joined = `${name}/${name}`;
      if (Object.prototype.hasOwnProperty.call(map, joined)) return joined;
      if (Object.prototype.hasOwnProperty.call(map, name)) return name;
      return null;
    };
    const k = resolveKey(variant.internalName, this.variantIdsValue);
    const vid = k ? this.variantIdsValue[k] : null;

    {
      const el = document.createElement('input');
      el.type = 'hidden';
      el.name = `product[variants_attributes][${index}][id]`;
      el.value = vid ? String(vid) : "";
      inputs.push(el);
    }
  ...
  prepareStockInputs(variantElement, internalName, index) {
    const resolveKey = (name, map) => {
      if (!map || !name) return null;
      const joined = `${name}/${name}`;
      if (Object.prototype.hasOwnProperty.call(map, joined)) return joined;
    ...
    const key = resolveKey(internalName, this.stockValue);
    const stockMap = key ? this.stockValue[key] : {};

    const stockLocations = (this.stockLocationsValue || []).map(String);

    stockLocations.forEach((locId, i) => {
      const countInput = variantElement.querySelector(`input[data-slot="[stock_items_attributes][${locId}][count_on_hand]_input"]`);
      const locInput   = variantElement.querySelector(`input[data-slot="[stock_items_attributes][${locId}][stock_location_id]_input"]`);
      const idInput    = variantElement.querySelector(`input[data-slot="[stock_items_attributes][${locId}][id]_input"]`);

      if (countInput) countInput.name = `product[variants_attributes][${index}][stock_items_attributes][${i}][count_on_hand]`;
      if (locInput)   { locInput.name = `product[variants_attributes][${index}][stock_items_attributes][${i}][stock_location_id]`; locInput.value = locId; }
      if (idInput)    idInput.name    = `product[variants_attributes][${index}][stock_items_attributes][${i}][id]`;

      const existing = stockMap && stockMap[locId];
      if (existing) {
        if (idInput && existing.id) idInput.value = String(existing.id);
        if (countInput && (countInput.value === "" || countInput.placeholder)) {
          if (existing.count_on_hand != null) countInput.value = String(existing.count_on_hand);
        }
      }
    });
  }

  preparePriceInputs(variantElement, internalName, index) {
    const resolveKey = (name, map) => {
      if (!map || !name) return null;
      const joined = `${name}/${name}`;
      if (Object.prototype.hasOwnProperty.call(map, joined)) return joined;
    ...
    const currencies = (this.currenciesValue || []).map(String);
    const key = resolveKey(internalName, this.pricesValue);
    const priceMap = key ? this.pricesValue[key] : {};

    currencies.forEach((currency, currencyIndex) => {
      const upper = currency.toUpperCase();
      const lower = currency.toLowerCase();

      const amountInput = variantElement.querySelector(`input[data-slot="[prices_attributes][${upper}][amount]_input"]`);
      const currencyInput = variantElement.querySelector(`input[data-slot="[prices_attributes][${upper}][currency]_input"]`);
      const idInput = variantElement.querySelector(`input[data-slot="[prices_attributes][${upper}][id]_input"]`);

      if (amountInput) amountInput.name = `product[variants_attributes][${index}][prices_attributes][${currencyIndex}][amount]`;
      if (currencyInput) {
        currencyInput.name = `product[variants_attributes][${index}][prices_attributes][${currencyIndex}][currency]`;
        currencyInput.value = currency;
      }
      if (idInput) idInput.name = `product[variants_attributes][${index}][prices_attributes][${currencyIndex}][id]`;

      const existing = priceMap && priceMap[lower];
      if (existing) {
        if (idInput && existing.id) idInput.value = String(existing.id);
        if (amountInput && (amountInput.value === "" || amountInput.placeholder)) {
          if (existing.amount != null) amountInput.value = String(existing.amount);
        }
      }
    });
  }
